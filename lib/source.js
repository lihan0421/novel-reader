...
import { decodeHtml, stripTags } from './textUtils.js';
...
function parseHotList(html) {
  const items = [];
  const re =
    /<li><span class="s1">\[([^\]]*)\]<\/span><span class="s2"><a href="\/book\/(\d+)\/">([^<]+)<\/a><\/span><span class="s5"><a[^>]*>([^<]*)<\/a><\/span><\/li>/g;
  let m;
  while ((m = re.exec(html))) {
    items.push({ id: m[2], title: m[3].trim(), intro: `[${m[1]}] ${m[4].trim()}` });
  }
  return items;
}
...
export async function discover() {
  ...
  return parseHotList(html).slice(0, 30);
}
...
export async function getBookInfo(bookId) {
  ...
  return {
    ...
    firstChapterUrl: readUrlMatch[1],
  };
}
...
export async function fetchChapterPage(url) {
  ...
  while ((m = re.exec(html))) {
    ...
    const text = stripTags(decoded).trim();
